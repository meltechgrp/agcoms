...
export type PropsMap = {
	...
	[EmailTemplate.RequestTemplate]: ComponentProps<typeof RequestTemplate>;
};
...
const getEmailTemplate = <T extends EmailTemplate>(
	...
	switch (template) {
		...
		case EmailTemplate.RequestTemplate:
			return {
				subject: 'New Request',
				body: render(
					<RequestTemplate
						{...(props as PropsMap[EmailTemplate.RequestTemplate])}
					/>
				),
			};
		default:
...
export const sendMail = async <T extends EmailTemplate>(
	...
	return transporter.sendMail({ from: EMAIL_SENDER, to, subject, html: res });
};
