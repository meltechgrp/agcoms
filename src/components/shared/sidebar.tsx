...
import useNavStore, { Key, keys } from '@/stores/use-nav-store';
									...
									<>
										{page.slug === 'product' ? (
											...
											<>
												{page.slug === 'financing' && (
													<div>
														<Link key={uniqueId()} href={`/${page.slug}`}>
															<SheetClose
																onClick={() => setState1(false)}
																className="text-sm capitalize flex items-center px-2 font-semibold">
																{page.slug}
															</SheetClose>
														</Link>
													</div>
												...
												<ul className="grid">
													{keys.includes(page.slug) &&
														store[page.slug as Key].map((c) => (
															<Link
																key={uniqueId()}
																href={c?.link ?? `/${page.slug}/${c.slug}`}>
																<SheetClose
																	onClick={() => setState1(false)}
																	className=" px-2 h-8 flex items-center hover:bg-gray-200 text-nowrap text-sm text-blue-700 font-medium">
																	{c.name}
